...
export class ZapHelper {
  ...
  static getZapTokenAllowanceHelper = async (tokenAddress: string, ownerAddress: string): Promise<boolean> => {
    ...
    const apiKey = EnvHelper.getZapperAPIKey();
    const response = await fetch(
      `https://api.zapper.fi/v1/zap-in/pool/sushiswap/approval-state?api_key=${apiKey}&ownerAddress=${ownerAddress}&sellTokenAddress=${tokenAddress}`,
    );
    const responseJson = await response.json();
    if (response.ok) {
  ...
  static changeZapTokenAllowanceHelper = async (
    ...
    const apiKey = EnvHelper.getZapperAPIKey();
    const response = await fetch(
      `https://api.zapper.fi/v1/zap-in/pool/sushiswap/approval-transaction?api_key=${apiKey}&ownerAddress=${ownerAddress}&sellTokenAddress=${tokenAddress}&gasPrice=${gasPrice}`,
    );
    const responseJson = await response.json();
    if (response.ok) {
      return responseJson;
  ...
  static executeZapLPHelper = async (
    sellAmount: number,
    ownerAddress: string,
    tokenAddress: string,
    ...
    const apiKey = EnvHelper.getZapperAPIKey();
    const response = await fetch(
      `https://api.zapper.fi/v1/zap-in/pool/sushiswap/transaction?ownerAddress=${ownerAddress}&network=ethereum&sellAmount=${sellAmount}&sellTokenAddress=${tokenAddress}&poolAddress=${EnvHelper.getZapperPoolAddress()}&payoutTokenAddress=${EnvHelper.getZapperPoolAddress()}&slippagePercentage=${slippagePercentage}&gasPrice=${gasPrice}&api_key=${apiKey}`,
    );
    const responseJson = await response.json();
    if (response.ok) {
      return responseJson;
